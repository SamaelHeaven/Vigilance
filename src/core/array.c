...
static Handle *handle_mark_readonly(Handle *handle) { return (Handle *) ((uintptr_t) handle | READONLY_TAG); }

static Handle *handle_decode(Handle *handle) { return (Handle *) ((uintptr_t) handle & ~READONLY_TAG); }

static bool handle_is_readonly(Handle *handle) { return ((uintptr_t) handle & READONLY_TAG) != 0; }
...
ReadonlyArray array_readonly(Array array) {
    Handle *handle = array.handle;
    assert(handle);
    if (!handle_is_readonly(handle)) {
        array.handle = handle_mark_readonly(handle);
    }
    return array;
}
...
bool array_is_readonly(const Array array) {
    assert(array.handle);
    return handle_is_readonly(array.handle);
}
